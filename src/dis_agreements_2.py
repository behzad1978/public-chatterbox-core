...
source_dir = home_dir + '/Chatterbox_UCL_Advance/Agreement_Disagreement/convs_1_2/'

#csv files containing line by line tweets in the form of seed/reply, seed/reply, ..
agreed_file_name = 'agreed_pairs_1_2'
disagreed_file_name = 'disagreed_pairs_1_2'
other_pairs_file_name = 'other_pairs'
...
def extract_training_and_test_data(y, x, all_seed_reply_texts):
    ...
    if len(y)==len(x):
        ...
        for i in range(0, length):
            if y[i]<>0:#do not consider pairs that are neither agreed nor disagreed.
    ...
    if len(y_test)==len(x_test):
        length=len(y_test)/2#We add 50% of other tweets that are neither agreed nor disagreed to the test set.
...
other_pairs = my_util.read_csv_file(source_dir + other_pairs_file_name, True)
