...
"""
This version adupts clustering algorithm (Pycluster) to cluster tokens with various prob vals.
"""
...
saving_dir = home_dir + '/chatterbox-core/twitter_param_results/Results12_qr'
...
str_comp_vals = [0.50, 0.75, 0.90]
duplicate_removal_flags = [True, False]
str_length_flags = ['longest', 'shortest']
...
def remove_duplicate_tweets(tweets_list):
    ...
    while len(tweets_list) > 0:
        ...
        #duplicates = [s for s in tweets_list if difflib.SequenceMatcher(None, s.lower(), t.lower()).quick_ratio() > 0.90]
        duplicates = [s for s in tweets_list if s.lower()==t.lower()]
...
header = [
    'stp_wrd_flag', 'sent_thresh_pos', 'sent_thresh_neg', 'no_of_tweets_pos', 'no_of_tweets_neg',
    'duplicates_removed_pos', 'duplicates_removed_neg','prob_plus_val', 'str_length_flag', 'nr_clusters',
    ...
    'str_comp_type', 'str_comp_val', 'final_toks_pos', 'final_toks_neg']
...
#               avg_prob_diff_neg, stdev_prob_diff_neg = math_extra.calc_mean_stdev(prob_diffs_neg)
                ...
                weighted_toks_pos = [w for w in weight_freq_pos if w[1] >= (avg_prob_pos + 1 * stdev_prob_pos)]
                weighted_toks_neg = [w for w in weight_freq_neg if w[1] >= (avg_prob_neg + 1 * stdev_prob_neg)]

                data_pos = [[v] for k, v in weighted_toks_pos]
                data_neg = [[v] for k, v in weighted_toks_neg]

                for nr_clusters in nClusters:

                    clusterids_pos = Pycluster.kcluster(data_pos, nr_clusters, None, None, 0, 1, 'a', 'e', None)
                    clusterids_neg = Pycluster.kcluster(data_neg, nr_clusters, None, None, 0, 1, 'a', 'e', None)

                    clusterids_pos = [x for x in clusterids_pos[0]]
                    clusterids_neg = [x for x in clusterids_neg[0]]
                    ...
                    for str_length_flag in str_length_flags:
                        ...
                        for str_comp_type in str_comp_types:
                            ...
                            for str_comp_val in str_comp_vals:
                                ...
                                final_toks_pos = topic_funcs.find_toks_by_clustering(
                                    weight_freq_pos, clusterids_pos, str_comp_type, str_comp_val, str_length_flag)

                                final_toks_neg = topic_funcs.find_toks_by_clustering(
                                    weight_freq_neg, clusterids_neg, str_comp_type, str_comp_val, str_length_flag)

                                final_toks_pos = sorted(final_toks_pos, key=operator.itemgetter(1))
                                final_toks_pos.reverse()
                                final_toks_pos = final_toks_pos[:no_of_presenting_tweets]

                                final_toks_neg = sorted(final_toks_neg, key=operator.itemgetter(1))
