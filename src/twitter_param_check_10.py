...
"""
In this version, we use background probability. In the older versions when the sentimental threshold values were
(0.5, -0.5), the tweets - and hence the tokens - having sentimental value between -0.5 < t < 0.5 are not used at all. In
this version these tweets are used as a background normalisation tool. The older probablity was p/(p+n) whereas the new
probability is p/(p+g+n) where g is the number of times that token t appears in tweets having sentimental value between
-0.5 < t < 0.5.
...
saving_dir = home_dir + '/twitter_param_results/Results10_qr'
...
sentiment_thresholds_pos_neg = [(0.40, -0.01), (0.01, -0.01), (0.25, -0.25), (0.50, -0.50)]
stop_word_flags = [True]#, False]
pickup_nums_1_and_2 = [(500, 100)]#, (500, 50), (250, 100), (250, 50)]
prob_plus_vals = [2, 3, 5]
stdev_mults = [0.125, 0.25, 0.5]#, 0.75, 1, 1.25, 1.5, 1.75, 2]
str_comp_types = ['quick_ratio']#, 'ratio', 'Levenshtein', 'stringComp']
str_comp_vals = [0.75]
duplicate_removal_flags = [True]# [False, True]
