...
collection_name = 'concerned' #'worried'
...
if remove_retweets:
    try:
        ...
        tweets = [' '.join(t.split())]#this part removes extra spaces that may exist between words.
    except IOError:
...
def create_feature_Vector(tweets, features_dict, features_count_dict, max_index, m, n):
    ...
    vectors = []
    for t in tweets:
        vector, tokens_length, max_index = funcs.ngrams2(t, features_dict, features_count_dict, max_index, m, n)
        vectors.append([vector, tokens_length])
    return vectors

print 'creating feature vectors...'
vectors_pos = create_feature_Vector(positives, features_dict_pos, features_count_dict_pos, max_index_pos, 1, 3)
vectors_neg = create_feature_Vector(negatives, features_dict_neg, features_count_dict_neg, max_index_neg, 1, 3)
